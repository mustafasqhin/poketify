...
async function loadTheme() {
	...
	document.querySelectorAll('.theme').forEach(theme => {
		theme.addEventListener('click', (evt) => {
			...
			localStorage.setItem('theme', THEME);
			showPopupMessage('Tema uygulandı');
			//make a request to the server to update the cookie
			const themeRequest = new XMLHttpRequest();
			themeRequest.open('PUT', '/theme');
			themeRequest.setRequestHeader('Content-Type', 'application/json');
...
function makeMessgaes(message, type, id, uid, reply, replyId, replyOptions, metadata) {
	...
	if (type === 'text') { //if the message is a text message
		...
		<div class='msg imageContainer'>
			...
			<div class="circleProgressLoader" style="stroke-dasharray: 0, 251.2;">
				...
				<div class="progressPercent">Yükleme bekleniyor</div>
			</div>
		</div>
		`; //insert the image
	...
	if (username == myName) { username = 'Siz'; }
	...
	if (replyOptions.reply) {
		...
		if (repliedTo == myName) { repliedTo = 'siz'; }
		if (repliedTo == username) { repliedTo = 'kendi'; }
		if (!document.getElementById(replyId)) {
			reply = { data: 'Mesaj bu cihazda mevcut değil', type: 'text' };
		}
	...
	*/
	...
	if (type === 'file') {
		...
		html = parseTemplate(fileTemplate, {
			...
			messageId: id,
			repId: replyId,
			title: replyOptions.reply ? `${username} bir mesaja ${repliedTo ? repliedTo : 'cevap verdi'}` : username,
			source: message,
			replyMsg: replyMsg,
		...
		*/
		...
		html = parseTemplate(messageTemplate, {
			...
			messageId: id,
			repId: replyId,
			title: replyOptions.reply ? `bir mesaja ${repliedTo ? repliedTo : 'cevap verdin'}` : username,
			message: message,
			replyMsg: replyMsg,
			time: getFormattedDate(timeStamp),
		});
...
export function deleteMessage(messageId, user) {
	...
	if (message) { //if the message exists
		...
		showPopupMessage(`${user == myName ? 'mesajı' : user} sildiniz`);
		...
		if (replyMessages != null) {
			replyMessages.forEach(reply => {
				...
				reply.textContent = 'Mesaj silindi';
			});
		}
...
function downloadHandler() {
	if (messageDatabase.get(targetMessage.id)?.loaded == false) {
		...
		if (targetMessage.sender == 'You') {
			showPopupMessage('Henüz yüklenmedi');
		} else {
			showPopupMessage('Henüz indirilmedi');
		}
		return;
	}
...
function saveImage() {
	try {
		...
		showPopupMessage('Resim hazırlanıyor...');
...
function downloadFile() {
	showPopupMessage('İndirmeye hazırlanıyor...');
...
function showReplyToast() {
	...
	if (!document.getElementById('replyToast')) {
		title.appendChild(document.createTextNode(' Yanıtlanıyor '));
...
function OptionEventHandler(evt, popup = true) {
	...
	try {
		...
		if (type == 'text') {
			...
			targetFile.fileName = targetMessage.message = 'Sesli mesaj';
...
function copyText(text) {
	...
	if (!navigator.clipboard) {
		showPopupMessage('Bu tarayıcı panoya erişimi desteklemiyor');
		return;
	}
	//copy the text to clipboard and show a popup
	navigator.clipboard.writeText(text);
	showPopupMessage('Panoya kopyalandı');
}
...
export function serverMessage(message, type = null, color = null) {
	...
	if (type == 'location') {
		...
		messageObj = {
			...
			text: `${message.user}' konumunu paylaştı`,
		};

		updateScroll('location', `${message.user}' konumunu paylaştı`);
	} else if (type == 'leave') {
		messageObj = {
			text: message.text
		};
...
document.querySelector('.quickSettingPanel').addEventListener('click', (evt) => {
	...
	//hideQuickSettings();
	localStorage.setItem('sendBy', messageSendShortCut);
	showPopupMessage('Ayarlar uygulandı');
});
...
document.getElementById('messageSound').addEventListener('click', () => {
	...
	showPopupMessage('Mesaj sesleri ' + (getMessageSound() ? 'enabled' : 'disabled'));
});
...
document.getElementById('buttonSound').addEventListener('click', () => {
	...
	showPopupMessage('Buton sesleri ' + (getButtonSound() ? 'enabled' : 'disabled'));
});
...
document.getElementById('stickersKeyboard').addEventListener('click', e => {
	...
	if (e.target.tagName === 'IMG' && e.target.classList.contains('sendable')) {
		...
		if (!stickerIsValid(msg)) {
			serverMessage({ text: '.....!' }, 'info', 'yellow');
			return;
		}
...
document.getElementById('invite').addEventListener('click', async () => {
	...
	try {
		if (!navigator.share) {
			showPopupMessage('Bu tarayıcıda paylaşım desteklenmiyor');
			return;
		}
		...
		if (evt.target.classList?.contains('imageContainer')) {
			...
			if (messageObj.loaded == false) {
				if (messageObj.sender == myId) {
					showPopupMessage('Henüz gönderilmedi');
				} else {
					showPopupMessage('Henüz indirilmedi');
				}
				...
				if (!audio.paused) {
					...
					if (evt.offsetX < audioContainer.offsetWidth && evt.offsetX > 0) {
						...
						if (!isFinite(audioObj.duration)) {
							showPopupMessage('Lütfen sesin yüklenmesini bekleyin');
							return;
						}
			...
			if (targetElement) {
				try {
					...
					showPopupMessage('silinmiş mesaj');
				}
			} else {
				showPopupMessage('silinmiş mesaj');
			}
		...
		if (target) {
			...
			if (evt.target.classList.contains('close')) {
				...
				if (id) {
					...
					selectedFilesCount.textContent = `seçili öğe ${selectedFileArray.length}`;
...
function playAudio(audioContainer) {
	...
	try {
		...
		if (!messageObj.src) {
			showPopupMessage('Ses oynatılmaya hazır değil');
			return;
		}
...
function fileIsAcceptable(file, type) {
	if (file.size > 100 * 1024 * 1024) {
		showPopupMessage('Dosya boyutu 100 mb`den az olmalıdır');
		return false;
	}
	...
	if (type == 'audio') {
		...
		if (!supportedAudioFormats.includes(file.type)) {
			showPopupMessage('Ses biçimi desteklenmiyor. Dosya olarak deneyin.');
			return false;
		}
	} else if (type == 'image') {
		const supportedImageFormats = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
		if (!supportedImageFormats.includes(file.type)) {
			showPopupMessage('Resim biçimi desteklenmiyor. Dosya olarak deneyin.');
			return false;
		}
	}
	return true;
...
function ImagePreview(filesFromClipboard = null) {
	...
	if (files.length > 10) {
		showPopupMessage('Bir seferde en fazla 10 resim gönderilebilir');
		return;
	}
	...
	try {
		for (let i = 0; i < files.length; i++) {
			...
			imageFragment.onerror = () => {
				...
				showPopupMessage('Dosya okunurken hata oluştu');
				clearFileFromInput();
				clearTargetMessage();
				clearFinalTarget();
			};
		...
		console.log(e);
		showPopupMessage('Resim okunurken hata oluştu');
		clearFileFromInput();
		clearTargetMessage();
		clearFinalTarget();
	}
...
function FilePreview(filesFromClipboard = null, audio = false) {
	...
	if (files.length > 10) {
		showPopupMessage('En fazla 10 dosya seçin');
		return;
	}
...
window.addEventListener('offline', function () {
	...
	document.querySelector('.offline .text').textContent = 'Çevrimdışısınız!';
...
window.addEventListener('online', function () {
	...
	document.querySelector('.offline .text').textContent = 'Tekrar çevrimiçi oldun!';
...
sendButton.addEventListener('click', (e) => {
	...
	if (recorderElement.dataset.recordingstate === 'true') {
		showPopupMessage('Lütfen önce kaydı durdurun');
		return;
	}
...
function closeFilePreview() {
	...
	setTimeout(() => {
		...
		selectedFilesCount.textContent = '0 öğe seçildi';
	}, 100);
}
						...
						if (xhr.readyState === XMLHttpRequest.OPENED) {
							...
							if (xhr.status === 200) {
								...
								if (elem) {
									...
									progressText.textContent = 'Bitirme...';
								...
								progressText.textContent = 'Yükleme başarısız oldu';
				...
				xhr.upload.onprogress = function (e) {
					if (e.lengthComputable) {
						...
						if (progressLog) {
							...
							if (progress === 100) {
								progressLog.textContent = 'tamamlanıyor...';
							}
						}
					}
				};
...
export function notifyUser(message, username, avatar) {
	if (('Notification' in window) && Notification.permission === 'granted') {
		...
		if (!document.hasFocus()) {
			document.querySelector('title').text = `${username} mesaj attı`;
			if (newMsgTimeOut == undefined) {
				newMsgTimeOut = setTimeout(() => {
					document.querySelector('title').text = 'Gelen kutusu';
...
document.getElementById('send-location').addEventListener('click', () => {
	...
	if (!navigator.geolocation) {
		showPopupMessage('Tarayıcınız coğrafi konumu desteklemiyor.');
		return;
	}
	navigator.geolocation.getCurrentPosition((position) => {
		if (!show) return;
		showPopupMessage('Konumunuzu takip etme...');
	...
	locationTimeout = setTimeout(() => {
		showPopupMessage('Hata oluştu. Lütfen tekrar deneyin.');
		locationTimeout = undefined;
		show = false;
	}, 5000);
});
...
cancelVoiceRecordButton.addEventListener('click', () => {
	...
	if (recorderElement.dataset.recordingstate === 'true') {
		//stop recording
		stopRecording();
		showPopupMessage('Sesli mesaj iptal edildi');
	}
	...
	recordButton.dataset.playstate = 'oynat';
...
function startRecordingAudio() {
	...
	navigator.mediaDevices.getUserMedia({ audio: true })
		...
		.catch(function (err) {
			...
			showPopupMessage('Mikrafona izin vermeniz gerekir.');
		});
}
...
function sendAudioRecord() {
	...
	fetch(recordedAudio.src)
		...
		.then(audioBlob => {
			...
			const file = new File([audioBlob], `Poketify-recording-${Date.now()}.mp3`, { type: 'audio/mpeg' });
...
document.addEventListener('DOMContentLoaded', () => {
	try {
		loginTimeout = setTimeout(() => {
			...
			preload.querySelector('.text').textContent = 'giriş yapmak';
		...
		slowInternetTimeout = setTimeout(() => {
			...
			const preload = document.getElementById('preload');
			preload.querySelector('.text').textContent = 'Yavaş internet';
...
(() => {
	...
	window.onpopstate = () => {
		...
		if (exitpressed && timeDifference <= 1000) {
			showPopupMessage('Çıkmak için lütfen oturumu kapatın');
			exitpressed = false;
		} else {
			exitpressed = true;
