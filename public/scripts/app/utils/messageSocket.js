...
chatSocket.on('connect', () => {
	...
	chatSocket.emit('join', params, function (err) {
		...
		if (err) {
			...
			showPopupMessage('Sunucuya bağlandı');
			...
			if ('Notification' in window) {
				...
				showPopupMessage('Tarayıcınız bildirimleri desteklemiyor');
			}
		}
	});
});
		...
		const userFragment = fragmentBuilder(
			{
				...
				childs: [
					...
					{
						tag: 'span',
						text: user.uid == myId ? user.username + ' (Siz)' : user.username,
					},
				],
			}
		);
...
chatSocket.on('disconnect', () => {
	...
	serverMessage({ color: 'grey', text: 'Sunucudan bağlantı kesildi😔', id: id }, 'disconnect');
	playLeaveSound();
	showPopupMessage('Sunucudan bağlantı kesildi');
});