...
app.get('/', (req, res) => {
    ...
    const cookie = req.cookies['theme'] || 'ocean';
    ...
    if (validateKey(req.params.key)) {
        if (keyStore.hasKey(req.params.key)) {
            ...
            if (keyStore.getKey(req.params.key).isFull()) {
                blockNewChatRequest(req, res, { title: 'Yetkisiz', errorCode: '401', errorMessage: 'Erişim engellendi', buttonText: 'Geri', icon: 'block.png' });
                return;
            }
            ...
            blockNewChatRequest(req, res, { title: 'Mevcut değil', errorCode: '404', errorMessage: 'Anahtar mevcut değil', buttonText: 'Geri', icon: 'error.png' });
            return;
        }
    }
    else {
        blockNewChatRequest(req, res, { title: 'Geçersiz', errorCode: '400', errorMessage: 'Anahtar geçerli değil', buttonText: 'Geri', icon: 'error.png' });
        return;
    }
});
...
app.post('/~', (req, res) => {
    ...
    console.log(referar);
    if (!referar) {
        blockNewChatRequest(req, res, { title: 'Yönlendiren Yok', errorCode: '403', errorMessage: 'Yönlendiren bulunamadı', buttonText: 'Geri', icon: 'error.png' });
        return;
    }
    if (req.headers.host != referar.split('/')[2]) {
        blockNewChatRequest(req, res, { title: 'Geçersiz Yönlendiren', errorCode: '403', errorMessage: 'Geçersiz Yönlendiren', buttonText: 'Geri', icon: 'error.png' });
        return;
    }
    ...
    if (!key) {
        ...
        blockNewChatRequest(req, res, { title: 'Bulunamadı', errorCode: '498', errorMessage: 'Oturum Anahtarı bulunamadı', buttonText: 'Yenile', icon: 'session.png' });
        return;
    }
});
function blockNewChatRequest(req, res, data) {
    res.setHeader('Developer', DEVELOPER);
...
function approveNewChatRequest(req, res, data) {
    const nonce = crypto.randomBytes(16).toString('hex');
...
app.get('/offline', (req, res) => {
    blockNewChatRequest(req, res, { title: 'Çevrimdışı', errorCode: 'Oops!', errorMessage: 'Çevrimdışısınız:(', buttonText: 'Yenile', icon: 'offline.png' });
});
app.get('*', (req, res) => {
    blockNewChatRequest(req, res, { title: 'Sayfa bulunamadı', errorCode: '404', errorMessage: 'Sayfa bulunamadı', buttonText: 'Ana sayfa', icon: '404.png' });
});
//fire up the server
