...
chatSocket.on('connection', (socket) => {
    try {
        socket.on('join', (params, callback) => {
            ...
            socket.emit('server_message', { color: 'var(--secondary-dark)', text: 'Sohbete katıldınız🔥', id: srvID }, 'join');
            socket.broadcast.to(params.key).emit('server_message', { color: 'var(--secondary-dark)', text: `${params.name} sohbete katıldı🔥`, id: srvID }, 'join');
        });
        socket.on('message', (message, type, uId, reply, replyId, options, callback) => {
            ...
            if (isRealString(message)) {
                ...
                if (filtered.containsBadWords) {
                    //warn the user
                    socket.emit('server_message', { color: 'orange', text: 'Mesajınız saldırgan kelimeler içeriyor🚫', id: crypto.randomUUID() }, 'warn');
                }
                callback(id, null);
            }
        });
        ...
        socket.on('disconnect', () => {
            ...
            if (SocketIds[socket.id]) {
                ...
                const srvID = crypto.randomUUID();
                socket.broadcast.to(key).emit('server_message', { color: 'orangered', text: `${user.username} sohbetten ayrıldı🐸`, who: user.uid, id: srvID }, 'leave');
